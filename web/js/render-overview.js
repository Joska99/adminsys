import { $, esc, UI, agents, badgeForState, isActive, isStale, relTime, tsShort, sortRows, arrow, errLine, nextCronRel, SESSION_STALE_MS, sessionAge, fmtUsd } from "./core.js";
...
export function agentCard(a, extra = "") {
  ...
  // session freshness — would have caught the "sessions frozen" bug at a glance
  const la = a.sessions && a.sessions.last_active;
  const laSrc = (((a.sessions || {}).recent || [])[0] || {}).source || "";
  const lastActive = la
    ? `<div class="lastact">last session ${esc(relTime(la))}${laSrc ? ` · ${esc(laSrc)}` : ""}</div>`
    : `<div class="lastact">no sessions</div>`;
  ...
  return `<div class="panel agent-card ${health}">
    ...
    ${banner}
    <div class="stat">
      <div><div class="num">${(a.sessions && a.sessions.total) || 0}</div><div class="lbl">sessions</div></div>
    ...
    <div class="card-foot">
      ${lastActive}
      <div class="cardmeta">${(((a.kanban || {}).tasks_total) || 0).toLocaleString()} kanban tasks</div>
      <div class="cardmeta">${(((a.sessions || {}).by_source || {}).cron || 0)} cron · ${(((a.sessions || {}).by_source || {}).discord || 0)} discord sessions</div>
      <div class="cardmeta">7d · ${esc(fmtUsd((a.sessions && a.sessions.cost_7d) || 0))} spend · ${(((a.sessions && a.sessions.tokens_7d) || 0)).toLocaleString()} tokens</div>
    </div>
    ${extra}
  </div>`;
}
...
export function renderOverview() {
  ...
  let activeAgents = 0, totalSessions = 0, runningTasks = 0, cronTotal = 0, cronFailed = 0, spend7d = 0, tokens7d = 0;
  cardList.forEach(a => {
    activeAgents += (a.gateway && a.gateway.active_agents) || 0;
    totalSessions += (a.sessions && a.sessions.total) || 0;
    spend7d += (a.sessions && a.sessions.cost_7d) || 0;
    tokens7d += (a.sessions && a.sessions.tokens_7d) || 0;
  ...
  // PROFILES — top 5 profiles by session count (agent · profile · … · sessions)
  const profEntries = cardList.flatMap(a => {
    ...
    return (((a.profiles || {}).profiles) || []).map(pr => {
      ...
      const sessions = pr.sessions != null ? pr.sessions : 0;
      return { sessions, html: `<tr>
        <td>${esc(a.name)}</td>
        ...
        <td class="mono">${sessions}</td>
      </tr>` };
    });
  });
  const profRows = profEntries.sort((x, y) => y.sessions - x.sessions).slice(0, 5)
    .map(e => e.html).join("") || `<tr><td colspan="7" class="empty">no profiles</td></tr>`;
  const secProfiles = secHead("PROFILES", "top 5 profiles by sessions") +
  ...
  // RECENT SESSIONS — latest across all shown agents (uses state.db session rows)
  const allSess = [];
  cardList.forEach(a => ((a.sessions && a.sessions.recent) || []).forEach(s =>
    allSess.push({ agent: a.name, s })));
  allSess.sort((x, y) => {
    const xa = x.s.started_at || "", ya = y.s.started_at || "";
    return xa < ya ? 1 : xa > ya ? -1 : 0;   // newest first
  });
  const sessRows = allSess.slice(0, 8).map(({ agent, s }) =>
    `<tr><td>${esc(agent)}</td><td class="ts">${esc(tsShort(s.started_at))}</td>` +
    `<td class="mono">${esc(s.model || "—")}</td>` +
    `<td class="mono">${s.messages == null ? "—" : esc(s.messages)}</td>` +
    `<td class="mono">${s.cost_usd != null ? esc(fmtUsd(s.cost_usd)) : "—"}</td>` +
    `<td>${esc((s.title || "").slice(0, 48) || "—")}</td></tr>`
  ).join("") || `<tr><td colspan="6" class="empty">no sessions</td></tr>`;
  const secRecent = secHead("RECENT SESSIONS", "latest 8 across agents") +
    `<div class="panel ovsec"><table><thead><tr><th>agent</th><th>started</th><th>model</th><th>msgs</th><th>cost</th><th>title</th></tr></thead><tbody>${sessRows}</tbody></table></div>`;
  ...
  $("tab-overview").innerHTML = `
    ...
    <div class="topgrid">
      ...
      <div class="kpi kpi-ind clickable" data-goto="schedule" title="open Cron">
        ${LINK_ICON}
      ...
      <div class="kpi kpi-slate clickable" data-goto="sessions" title="open Sessions">
        ${LINK_ICON}
      ...
      <div class="kpi kpi-org">
        <span class="kpi-icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><g><path d="M30.47 28.95H32v1.53h-1.53Z"></path><path d="m30.47 1.52 -1.52 0 0 1.53 -1.52 0 0 1.52 1.52 0 0 1.53 1.52 0 0 -1.53 1.53 0 0 -1.52 -1.53 0 0 -1.53z"></path><path d="M28.95 22.86h1.52v1.52h-1.52Z"></path><path d="M27.43 24.38h1.52v1.53h-1.52Z"></path><path d="M27.43 21.33h1.52v1.53h-1.52Z"></path><path d="M27.43 15.24h1.52v1.52h-1.52Z"></path><path d="M27.43 9.14h1.52v1.53h-1.52Z"></path><path d="M25.9 22.86h1.53v1.52H25.9Z"></path><path d="M25.9 16.76h1.53v1.53H25.9Z"></path><path d="M25.9 7.62h1.53v1.52H25.9Z"></path><path d="M24.38 18.29h1.52v1.52h-1.52Z"></path><path d="M24.38 6.1h1.52v1.52h-1.52Z"></path><path d="m24.38 32 0 -1.52 1.52 0 0 -1.53 -1.52 0 0 -1.52 -1.53 0 0 1.52 -1.52 0 0 1.53 1.52 0 0 1.52 1.53 0z"></path><path d="M22.85 19.81h1.53v1.52h-1.53Z"></path><path d="M21.33 21.33h1.52v1.53h-1.52Z"></path><path d="M21.33 15.24h1.52v1.52h-1.52Z"></path><path d="M21.33 9.14h1.52v1.53h-1.52Z"></path><path d="M21.33 1.52h1.52v1.53h-1.52Z"></path><path d="M19.81 22.86h1.52v1