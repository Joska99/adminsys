import { $, esc, UI, agents, badgeForState, isActive, isStale, relTime, tsShort, sortRows, arrow, errLine, nextCronRel, SESSION_STALE_MS, sessionAge, fmtUsd, SNAP } from "./core.js";
...
export function agentCard(a, extra = "") {
  ...
  const lastActive = la
    ? `<div class="lastact">last main agent session ${esc(relTime(la))}${laSrc ? ` · ${esc(laSrc)}` : ""}</div>`
  ...
  let runningAll = 0, cronSess = 0, discordSess = 0, c7 = 0, c30 = 0, t7 = 0, t30 = 0;
  ...
  profs.forEach(p => {
    ...
    c7 += s.cost_7d || 0; c30 += s.cost_30d || 0;
    t7 += s.tokens_7d || 0; t30 += s.tokens_30d || 0;
  });
  ...
  const spark = d7.length
    ? `<div class="spark-wrap"><div class="spark-col"><div class="spark">${d7.map(x =>
...
export function renderOverview() {
  ...
  // TOP SKILLS — top 10 used per profile (profile skills/.usage.json use_count)
  const tsHtml = cardList.flatMap(a =>
    ((a.profiles && a.profiles.profiles) || []).map(pr => {
      const tu = pr.skills_used || [];
      ...
      return `<div class="panel"><h3><span class="agentname">${esc(a.name)}</span> <span class="pill mono profile-tag">${esc(pr.name)}</span></h3><table><thead><tr><th>#</th><th>skill</th><th>uses</th></tr></thead><tbody>${rows}</tbody></table></div>`;
    })).join("");
  const secTopSkills = secHead("TOP SKILLS", "top 10 used per profile") + `<div class="grid">${tsHtml}</div>`;

  // TOP TOOLS — most-called runtime tools per profile (profile state.db messages)
  const ttHtml = cardList.flatMap(a =>
    ((a.profiles && a.profiles.profiles) || []).map(pr => {
      const tt = pr.tools_top || [];
      const rows = tt.map((s, i) => `<tr><td class="muted">${i + 1}</td><td>${esc(s.name)}</td><td>${s.count}</td></tr>`).join("") || `<tr><td colspan="3" class="empty">no tool calls</td></tr>`;
      return `<div class="panel"><h3><span class="agentname">${esc(a.name)}</span> <span class="pill mono profile-tag">${esc(pr.name)}</span></h3><table><thead><tr><th>#</th><th>tool</th><th>calls</th></tr></thead><tbody>${rows}</tbody></table></div>`;
    })).join("");
  const secTopTools = secHead("TOP TOOLS", "top 10 called per profile") + `<div class="grid">${ttHtml}</div>`;
  ...
  const secTokens = secHead("TOKENS", "API responses · per-model · all-time (response_store.db)") +
  ...
  // TEMP tab — sections moved off Overview
  $("tab-temp").innerHTML = `
    ${secProfiles}
    <div class="midline"></div>
    ${secSchedule}
